...
julia> _, proms = peakprom!(pks, y; minprom=3)
...
- Fully supports `NaN`/`missing` with optional tolerance using keyword arg `strict`:
    - Conventional handling/propagation of `NaN`/`missing` when `strict = true` (the default)
      ...
      julia> peakprom([2,4], [NaN,2,0,1,1,0])
      ([2, 4], [NaN, 1.0])

      julia> peakwidth([2,4], [NaN,2,0,1,1,0], [2,1])
    ...
    - Reasonable alternatives when `strict = false`
      ...
      julia> peakprom([2,4], [NaN,2,0,1,1,0]; strict=false)
      ([2, 4], [2.0, 1.0])

      julia> peakwidth([2,4], [NaN,2,0,1,1,0], [2,1]; strict=false)
